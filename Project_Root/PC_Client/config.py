...
DEFAULT_STREAM_HOSTS = [
    host.strip()
    for host in os.environ.get("DEFAULT_STREAM_HOSTS", "boebot.local,192.168.4.1").split(",")
    if host.strip()
]
DEFAULT_STREAM_IP = os.environ.get(
    "DEFAULT_STREAM_IP", DEFAULT_STREAM_HOSTS[0] if DEFAULT_STREAM_HOSTS else ""
)
