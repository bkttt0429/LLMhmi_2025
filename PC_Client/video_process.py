# ============================================ 
# PC_Client/video_process.py Final Fixed Version
# Properly handles frame extraction without blocking
# ============================================ 
...
class MJPEGStreamReader:
    ...
    def get_frame(self):
        """Get single frame with automatic reconnection - FIXED VERSION"""
        ...
        try:
            ...
            while chunks_read < max_chunks_per_call:
                ...
                a = self.bytes.find(b'\xff\xd8')  # Start of Image (SOI)
                b = self.bytes.find(b'\xff\xd9')  # End of Image (EOI)

                if a != -1 and b != -1 and b > a:
                    # Extract the JPEG frame
                    jpg = self.bytes[a:b+2]
                    self.bytes = self.bytes[b+2:]  # Keep remaining bytes for next frame
                    ...
                    if frame is not None:
                        ...
                        # Decoding failed, discard this data and continue
                        print("[STREAM] ⚠️ Failed to decode frame, continuing...")
                        continue

            # If we've read max_chunks but still no complete frame, return None and try again next call
            return None
...
def video_process_target(cmd_queue, frame_queue, log_queue, initial_config):
    ...
    while True: 
        ...
        if frame is not None:
            ...
            # Only count as failure if we're connected but getting no frames
            if stream_reader and stream_reader.connected:
            ...
            if consecutive_failures >= 10:
                ...
                time.sleep(0.01)  # Small delay when no frame to avoid busy loop